...
sub check_options {
    Getopt::Long::Configure ("bundling");
        ...
        'd:s'   => \$o_dbname,          'dbname:s'              => \$o_dbname,
        'u:s'   => \$o_user,            'user:s'                => \$o_user,
                'V'             => \$o_version,         'version'               => \$o_version,
        'c:s'   => \$o_crit,            'critical:s'    => \$o_crit,
        'w:s'   => \$o_warn,            'warn:s'                => \$o_warn,
                'b:s'   => \$o_hours,           'backup-hours'  => \$o_hours,
                'f'     => \$o_perf,        'perfparse'     => \$o_perf,
    );
    if (defined ($o_help) ) { help(); exit $ERRORS{"UNKNOWN"}};
    if (defined($o_version)) { p_version(); exit $ERRORS{"UNKNOWN"}};
    if ( !defined($o_type) )
        { print "No check type defiend!\n";print_usage(); exit $ERRORS{"UNKNOWN"}};
    if ( ($o_type ne 'L') && ($o_type ne 'l') &&
         ($o_type ne 'D') && ($o_type ne 'd') &&
         ($o_type ne 'B') && ($o_type ne 'b') &&
                 ($o_type ne 'S') && ($o_type ne 's') )
        { print "wrong check type!\n";print_usage(); exit $ERRORS{"UNKNOWN"}};
    if ((( $o_type eq 'B' ) || ( $o_type eq 'b' ))  && !defined($o_hours))
        { print "No hours for backup history check defiend!\n";print_usage(); exit $ERRORS{"UNKNOWN"}};
    if ((( $o_type eq 'B' ) || ( $o_type eq 'b' ))  && defined($o_hours))
    ...
    if ( !defined($o_user))
        ...
        if (( $o_type ne 'S' ) && ( $o_type ne 's' )) {
            ...
            if ( ($o_crit > 100) || ($o_warn > 100) )
                { print "warning or critical <= 100! \n";print_usage(); exit $ERRORS{"UNKNOWN"}};
        }
}
####### Query the database #####
sub query {
